...
#define  ESP_WIFI_SSID "HUAWEI NOVA Y60"
...
lora_receive_packet(void)
{
   ...
   printf("param1: %d, param2: %d, param3: %0.2f-%s\n", m.dest, m.emitter, m.value1,m.msg);
   return len;
}
    ...
    case MQTT_EVENT_DATA:
        ...
        if(!(strcmp(topic_str,"test2"))){
            ...
            ESP_LOGE(TAG2, "Failed to parse JSON data");
                ...
                if (cJSON_GetObjectItem(root, "velnode1")) {
                    char *velnode1 = cJSON_GetObjectItem(root,"velnode1")->valuestring;
                    sprintf(velnode1Str,"%s",velnode1);                 
	            }
                ESP_LOGI(TAG2, "velgateway=%s    velnode1=%s",velgatewayStr,velnode1Str);
                cJSON_Delete(root);
...
static void mqtt_app_start(void)
{
    ...
    esp_mqtt_client_config_t mqttConfig = {
        .broker.address.uri = "mqtt://192.168.43.17",
...
void timer_callback(void *param)
{
  ...
  cJSON_AddNumberToObject(root, "value1", m.value1);
  cJSON_AddStringToObject(root, "msg", m.msg);
  char *my_json_string = cJSON_Print(root);
  ESP_LOGI(TAG2, "my_json_string\n%s",my_json_string);
  esp_mqtt_client_publish(client, "test",  my_json_string, 0, 0, 0);
  cJSON_Delete(root);
